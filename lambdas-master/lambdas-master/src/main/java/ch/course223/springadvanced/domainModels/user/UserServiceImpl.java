...
*/
    ...
    public List<User> allUsersEarningMoreThan(int x) {
        // SalaryOperationMultipleResults allUsersEarningMoreThan2000 =
        // (List<User> users) -> users.stream().filter(user -> user.getSalary() > 2000).collect(Collectors.toList());
    ...
    public int getAverage() {
        return userRepository.findAll().stream().filter() userRepository.findAll().stream().collect(Collectors.toList()).size();
    }
